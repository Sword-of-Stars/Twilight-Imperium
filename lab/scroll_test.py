...
class PlayerTracker:
    def __init__(self, players):
        ...
        self.width, self.height = 540, 430
...
class Player:
    def __init__(self, name, info):
        self.name = name
        self.info = info  # Multi-line string of player details
...
if __name__ == "__main__":
    ...
    player_tracker = PlayerTracker(players)
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                player_tracker.handle_mouse_wheel(pygame.mouse.get_pos(), event)
            elif event.type == pygame.MOUSEBUTTONDOWN:
                player_tracker.handle_mouse_click(pygame.mouse.get_pos())

        player_tracker.update()
        player_tracker.draw(screen)
